...
function comma_list(values, template1, ends1, template2, ends2) {
	...
	for (var c1 = 0; c1 < values.length; c1++) {
		...
		if (values[c1] == ".") {
			...
			var last = (c1 == values.length-1 || values[c1+1].charAt(0) == ".");
			if (t1c > 0) {
				t1c--;
				ss += "|" + values[c1];
				...
				ss += ll(values[c1]) + comma(values, c1, ends1, ends2);
			}
		}
	}
	return ss;
...
function comma(values, c, ends1, ends2) {
	if (c >= values.length -1) return "";
	
	var ends = false;
	
	if (values[c+1] == ".") ends = ends1;
	else if (values[c+1] == "..") ends = ends2;
	else return ", ";
