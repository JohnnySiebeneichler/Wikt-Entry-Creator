...
function makespc(type, data, first, last) {
	...
	if (first) {
		...
		ss += "|" + data[1] + "|" + data[2] + "|alt1=";
		ss += ((data[7] == "")? (data[1].substring(0, 1).toUpperCase() + data[1].substring(1) + (type == ".p"? "-" : "")) : data[7]);
		...
		ss += " from" + maketerm(data[0], data[1], data[7], data[3], data[10]);
...
function makeconfix(type, data, first, last) {
	var ss = "";
	if (first) {
		ss += "{{confix|" + data[1] + "|" + data[2] + "|" + data[3] + "|alt1=";
		ss += (data[7] == ""? (data[1].substring(0, 1).toUpperCase() + data[1].substring(1) + "-") : data[7]);
		if (data[8] != "") ss += "|alt2=" + data[8];
...
function maketerm(lang, term, alt, gloss, tr) {
	...
	var ss = "";
	ss += " {{" + term_or_recons(lang) + "|" + term;
	if (alt != "" || gloss != "") ss += "|" + (alt == term? "" : alt);
	if (gloss != "") ss += "|" + gloss;
	ss += "|lang=" + lang;
	if (tr != "") ss += "|tr=" + tr;
	if (get_script(lang) != "Latn") ss += "|sc=" + get_script(lang);
	return ss + "}}";
}
...
function term_or_recons(lang) {
	if (lang.charAt(2) == "." || lang.sub(4) == "pro") return "recons";
	return "term";
}
...
function auto_latin_lemma(data) {
	...
	var suffix5 = data[1].substring(data[1].length - 5);
	...
	if (suffix4 == "inem") {
		...
		lemma = data[7].substring(0, data[1].length - 5) + "ans";
		whatform = "accusative singular";
	}
		...
		lemma = data[7].substring(0, data[1].length - 3) + "er";
		whatform = "accusative singular";
	}
	else if (suffix2 == "us") {
		lemma = data[7].substring(0, data[1].length - 2) + "ō";
		whatform = "perfect passive participle";
	}
	else if (suffix2 == "re") {
