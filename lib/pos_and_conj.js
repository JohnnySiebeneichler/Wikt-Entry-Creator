...
function make_head_alt() {
	var type = has_efficient_l(l)? "/" : "|";
	var head = w.replace(/\s/g, ("}} {{l"+type+l+"|"));
	head = head.replace(/-/g, ("}}-{{l"+type+l+"|"));
	if (head == w) return "";
	return "|head={{l" + type + l + "|" + head + "}}";
}
...
function pt_noun_parameters() {
	...
	else if (suffix1 == "m") ss += w.substring(0, w.length-1) + "ns";
...
function pt_adj_parameters() {
	...
	if (w.substring(w.length-3) == "vel") {
		return ss + w.substring(0, w.length-4) + "|" + w.substring(w.length-4);
	}
	var suffix2 = w.substring(w.length-2);
